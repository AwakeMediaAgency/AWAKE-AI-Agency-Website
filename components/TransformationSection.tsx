    ...
    <section className="py-40 bg-brand-dark relative">
      <div className="container mx-auto px-6">
        <div className="text-center mb-32">
          <span className="inline-block text-brand-blue font-bold tracking-[0.4em] uppercase mb-6 text-sm">The Transformation</span>
          <h2 className="text-6xl md:text-9xl font-black tracking-tighter text-white uppercase italic leading-[0.85]">
            MAKING THE <br />
            <span className="text-brand-blue">OUTCOME TANGIBLE.</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {STAGES.map((stage, idx) => (
            <div key={idx} className="relative p-12 glass rounded-[50px] border-white/5 flex flex-col items-center text-center group">
              <div className="mb-10 p-6 bg-white/5 rounded-3xl text-brand-blue group-hover:bg-brand-blue group-hover:text-white transition-all duration-500">
                {stage.icon}
              </div>
              <h4 className="text-3xl font-black mb-4 uppercase italic tracking-tighter">{stage.title}</h4>
              <span className="inline-block px-4 py-1.5 bg-brand-blue/10 text-brand-blue rounded-full text-xs font-bold uppercase tracking-widest mb-6">
                {stage.time}
              </span>
