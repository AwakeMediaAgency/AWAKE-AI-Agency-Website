...
const SocialProof: React.FC = () => {
  return (
    <section className="py-32 px-6 bg-[#050505] relative overflow-hidden">
      {/* Decorative gradient */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[400px] bg-[#1f75fe]/5 blur-[120px] rounded-full pointer-events-none"></div>

      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-24">
          <h2 className="text-xs font-bold uppercase tracking-[0.5em] text-[#1f75fe] mb-6">Success Metrics</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {TESTIMONIALS.map((t, idx) => (
            <div key={idx} className="relative group">
              <div className="absolute -top-6 -left-4 text-[#1f75fe]/10 group-hover:text-[#1f75fe]/20 transition-colors">
              ...
              <div className="p-10 glass rounded-[40px] border border-white/5 h-full flex flex-col relative z-10">
                <div className="flex gap-1 mb-8">
                  {[...Array(5)].map((_, i) => <Star key={i} className="w-4 h-4 fill-[#1f75fe] text-[#1f75fe]" />)}
                </div>
                ...
                <div className="mt-auto flex items-center justify-between">
                  ...
                  <div className="px-4 py-2 rounded-xl bg-[#1f75fe]/10 border border-[#1f75fe]/20 text-[#1f75fe] text-xs font-black uppercase">
                    {t.metric}
                  </div>
                </div>
              </div>
